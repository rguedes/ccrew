...
class Wn8 {
     ...
     *
     * @param   \RiftBit\WoTAPI\Api   $api                            Wargaming\API instance version at least 1.04. It is used to retrieve data from Wargaming servers.
     ...
     * @throws \Exception
     */
    public function __construct($api, $search, $accurate_calculation = false, $missing_search = false ) {
        $this->api = $api;
        ...
        if( is_string($search) && is_nan($search) ) {

            // Get account from Wargaming servers
            $account = $this->api->accountList($search, 'account_id', 'exact');
    ...
    protected function loadExpectedTankValues() {
        ...
        $expected_tanks = Cache::get('expected_tank_values', null);
        if(is_null($expected_tanks)){
            $expected_tanks = json_decode(file_get_contents(resource_path('assets/wn8exp.json')));
            Cache::put('expected_tank_values', $expected_tanks, 60*24);
        }

        // Load expected tank values
        $buff = $expected_tanks->data;
    ...
    private static function _arrayToObject($array) {
    ...
    public function getWN8() {
        ...
        if( is_null($this->wn8) ) {
            ...
            // Get summary values
            $summary = $this->api->accountInfo($account_id,'statistics.all.battles,statistics.all.frags,statistics.all.damage_dealt,statistics.all.dropped_capture_points,statistics.all.spotted,statistics.all.wins');
            $summary = $this->_arrayToObject($summary['data'][$account_id])->statistics->all;

            // Get tanks values
            $tanks = $this->api->accountTanks($account_id, "", 'tank_id,statistics.battles')['data'][$account_id];
            ...
            if( !empty($missing) AND $this->search_missing_tanks ) {
                $this->missing_tanks = $this->api->encyclopediaVehicles('name', "", "", "",implode(',',$missing))['data'];
            }
            ...
            Cache::put('wn8_'.$this->account_id, $this->wn8, 60*24);
        }
